...
# 注入 vmlinux-btf 极速独立 BTF 生成包 (外挂 RootFS，零侵占 6MB U-Boot 分区)
mkdir -p "$PKG_PATH/vmlinux-btf"
cat <<'EOF_BTF' > "$PKG_PATH/vmlinux-btf/Makefile"
#
# Copyright (C) 2026 OpenWrt.org
#
# Standalone detached BTF generator for CO-RE eBPF (dae/daed)
#
...
define Package/vmlinux-btf/description
  Supplies detached kernel BTF information to ensure CO-RE eBPF compatibility
  for dae/daed without inflating the kernel partition size.
endef
...
define Build/Compile
	if command -v pahole >/dev/null 2>&1 && [ -f "$(LINUX_DIR)/vmlinux" ]; then \
		echo "Generating detached BTF from $(LINUX_DIR)/vmlinux with pahole..."; \
		pahole --jobs=$(PAHOLE_JOBS) --btf_encode_detached=$(PKG_BUILD_DIR)/vmlinux-btf $(LINUX_DIR)/vmlinux || { \
			echo "Warning: pahole detached BTF encoding failed!"; \
			exit 1; \
		}; \
	else \
		echo "Error: pahole or $(LINUX_DIR)/vmlinux missing!"; \
		exit 1; \
	fi
endef
...
define Package/vmlinux-btf/install
	...
	[ -z "$(LINUX_UNAME_VERSION)" ] || $(LN) vmlinux $(1)/usr/lib/debug/boot/vmlinux-$(LINUX_UNAME_VERSION)
endef

$(eval $(call BuildPackage,vmlinux-btf))
EOF_BTF
echo "vmlinux-btf package recipe created successfully!"
